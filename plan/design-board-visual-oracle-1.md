...
| TASK-010 | Align `catalog-site/src/pages/cookbooks/index.astro` and `catalog-site/src/components/Cookbooks.ts` with `board-cookbooks.jsx`: compact category tabs, bordered cookbook cards, stack selector, dark code block, disclosed persona fit check, and hard-shadow handoff modal. Preserve the truthful standard Copilot CLI command already implemented. | | |
