...
assert_contains "certificate.sh --apply installs the pinned certbot spec" "$(cat "$ledger")" "certbot==5.6.0"
assert_contains "certificate.sh --apply installs the matching pinned acme spec" "$(cat "$ledger")" "acme==5.6.0"
assert_contains "certificate.sh --apply installs the pinned certbot-dns-cloudflare spec" "$(cat "$ledger")" "certbot-dns-cloudflare==5.6.0"
...
assert_contains "certificate.sh --apply uses Azure CLI's supported certificate password flag" "$(cat "$ledger")" "--password"
assert_not_contains "certificate.sh --apply never uses the unsupported legacy password flag" "$(cat "$ledger")" "--certificate-password"
