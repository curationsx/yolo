...
# Non-secret ACME account contact email for certificate.sh's real DNS-01
# issuance (--email passed to certbot; Let's Encrypt requires either this
# or --register-unsafely-without-email). Overridable, never a secret.
: "${YOLO_ACME_CONTACT_EMAIL:=ops@curations.dev}"
...
export YOLO_ACME_CONTACT_EMAIL
